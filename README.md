...
**AUDUSD 30m OHLCV forex historical data** — ultra high-quality 30m OHLCV for **Australian Dollar / US Dollar**. Clean `time, open, high, low, close, volume` CSV for backtesting, algorithmic trading and quantitative research.
...
> **Sample on GitHub** · `AUDUSD_30m.csv` (1,850 rows, `2026-07-09` -> `2026-09-02`, 188.24 KB). **Full archive on [getdata.finance](https://getdata.finance/datasets/audusd)** — **175,681** `30m` rows (full `1m`: 5,263,593), **11 timeframes**, `2012-06-24` -> `2026-09-02`.
...
| Size | 188.24 KB | full ZIP on [getdata.finance](https://getdata.finance/datasets/audusd) |
...
df = pd.read_csv('AUDUSD_30m.csv', parse_dates=['time'])
